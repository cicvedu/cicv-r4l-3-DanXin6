...
use alloc::borrow::{BorrowMut, ToOwned};
use bindings::{
    dev_get_drvdata, dma_unmap_resource, free_irq, get_options, hwrng_register, pci_get_device,
    unregister_netdev,
};
use kernel::device::RawDevice;
use kernel::driver::{DeviceRemoval, DriverOps};
...
impl net::DeviceOperations for NetDevice {
    ...
    fn stop(dev: &net::Device, data: &NetDevicePrvData) -> Result {
        ...
        let mut tx_ring = data.tx_ring.lock_irqdisable();
        let mut rx_ring = data.rx_ring.lock_irqdisable();
        rx_ring.deref_mut().take();
        tx_ring.deref_mut().take();
...
impl pci::Driver for E1000Drv {
    ...
    fn probe(dev: &mut pci::Device, id: core::option::Option<&Self::IdInfo>) -> Result<Self::Data> {
        ...
        // (*self.dev).dev
        let net_device_prv_data = Box::try_new(NetDevicePrvData {
        ...
        netdev_reg.register(net_device_prv_data)?;
    ...
    fn remove(dev: &mut pci::Device, data: &Self::Data) {
        // let device:&net::Device = (data._netdev_reg.borrow_mut().dev_get().deref());
        // net::DeviceOperations::stop(device, dev_get_drvdata(dev));
...
impl Drop for E1000KernelMod {
    fn drop(&mut self) {
        ...
        unsafe {Pin::as_mut(&mut self._dev).get_unchecked_mut()};
    }
}
